...
def load_Ogbn(dataset='ogbn-arxiv', root='./data/'):
    if dataset == 'ogbn-arxiv':
        subgraph_rate = 0.25
        data = DglNodePropPredDataset(name=dataset, root=root+dataset)
        ...
        print('num_node before sampling ', num_node)

        num_node = int(subgraph_rate * num_node)
        sample_nodes = [i for i in range(num_node)]

        idx_train = idx_train[idx_train < num_node]
        print("train_num =", len(idx_train), end='; ')
        idx_val = idx_val[idx_val< num_node]
        print("val_num =", len(idx_val), end='; ')
        idx_test = idx_test[idx_test< num_node]
        print("test_num =", len(idx_test))

        g = dgl.node_subgraph(g, sample_nodes, relabel_nodes=True)
        labels = labels[sample_nodes]
        features = features[sample_nodes, :]
        print('num_node after sampling ', features.shape[0])
        ...
        print('num_edges after sampling without self-loop', (adj>0).sum() - num_node)
    ...
    return graph, labels, adj, features, idx_val, idx_test, idx_train
...
def load_syn_cora(dataset, path, homophily_ratio_name, seed):
    from data.dataset import CustomDataset
    dataset = CustomDataset(root=path + dataset, name=homophily_ratio_name, setting="gcn", seed=seed)
    ...
    random_state = np.random.RandomState(seed=seed)
    idx_train, idx_val, idx_test = get_train_val_test_split(random_state, encode_onehot(labels.tolist()), 20, 30)
