...
from utils import load_CoraFull, load_Coauthor, load_Amazon, encode_onehot, get_train_val_test_split, normalize_adj,load_data
...
def main(order):
    ...
    class MLP1(nn.Module):
        def __init__(self):
            ...
            self.mlp = MLP(nfeat=int(features.shape[1]), nhid=nhid, nclass=int(np.argmax(labels)+1),
...
for name in ['cora','citeseer','pubmed']:
    ...
    graph, labels, adj, features, idx_val, test_mask, train_mask, y = load_data(name, './data/'+name)
