...
from utils import load_data, accuracy, load_large_dataset, sample_per_class, encode_onehot
...
for d in ['cora', 'citeseer']:
    ...
    if d == 'cora':
        ...
        args.order = 2
    for nepc in [2,5,10]:
        args.num_example_per_class = nepc
        for j in range(times):
            ...
            if acc < min:
                min = acc
            avg += acc
            with open('./' + args.dataset + '_grand_label_rate{}_'.format(args.num_example_per_class) + '.csv', 'a+') as f:
